...
async function main(){
    ...
    let transaction = await escrow.connect(seller).list(1, tokens(5),buyer.address, tokens(10));
    await transaction.wait();
    transaction = await escrow.connect(seller).list(2, tokens(20),buyer.address, tokens(5));
    await transaction.wait();
    transaction = await escrow.connect(seller).list(3, tokens(10),buyer.address, tokens(15));
    await transaction.wait();
